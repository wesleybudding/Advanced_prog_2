        ...
        for (int i = 0; i < in.length(); i++) {
            ...
            if ((next == '+') || (next == '-') || (next == '|')) {
                ...
                for (int j = i + 1; j < in.length(); j++) {
                    char nextChar = in.charAt(j);
                    if ((nextChar == '+') || (nextChar == '-') || (nextChar == '|')) {
                        term2 = in.substring(i + 1, j);
                        rest = in.substring(j + 1);

                        Set<BigInteger> set1 = processTerm(term);
                        Set<BigInteger> set2 = processTerm(term2);
                        ...
                        if (next == '+') {
                            ...
                            result = set1.indifference(set2);
                        }

                        Set<BigInteger> set3 = processExpression(rest);

                        if (nextChar == '+') {
                            return result.union(set3);
                        } else if (nextChar == '-') {
                            return result.complement(set3);
                        } else if (nextChar == '|') {
                            return result.indifference(set3);
                        }
    ...
    public void start() throws APException {
        ...
        while (in.hasNextLine()) {
            ...
            if (line.equals("q")) {
                ...
                System.out.println("No data - Enter data: (q to quit)");
            }
        }
    }
